...
class TemplateManager: NSObject, ObservableObject, CPInterfaceControllerDelegate, CPSessionConfigurationDelegate  {
    //class TemplateManager: NSObject, ObservableObject, CLLocationManagerDelegate  {
    ...
    func connect(_ interfaceController: CPInterfaceController, scene: CPTemplateApplicationScene) {
        ...
        lm.locationManager.pausesLocationUpdatesAutomatically = false
        lm.locationManager.startMonitoringSignificantLocationChanges()
        
        Task {
            await  wvm.getMoreWeather()
            
        }
        
        let listItem = CPListItem(text: "\(speedString) MPH", detailText: directionString)
    ...
    func disconnect() {
        ...
        lm.locationManager.stopUpdatingHeading()
        
        lm.locationManager.allowsBackgroundLocationUpdates = false
        lm.locationManager.pausesLocationUpdatesAutomatically = true
        lm.locationManager.stopMonitoringSignificantLocationChanges()
    }
